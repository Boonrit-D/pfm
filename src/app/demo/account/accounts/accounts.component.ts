...
export class DemoAccountsComponent implements OnInit {
  ...
  */
  constructor(private demoCrudService: DemoCrudService) {}
  ...
  */
  ngOnInit(): void {
    this.hoveredIndexes = new Array(this.demoAccounts.length).fill(false);
  ...
  setRandomImagesAndBackgroundColors(): void {
    ...
    this.demoAccounts.forEach((account: any, index: number) => {
  ...
  deleteAccount(id: any, i: any) {
    if (window.confirm('คุณต้องการดำเนินการลบใช่หรือไม่?')) {
      this.demoCrudService.deleteAccount(id).subscribe((res) => {
        this.demoAccounts.splice(i, 1);
      });
    }
  }
}
